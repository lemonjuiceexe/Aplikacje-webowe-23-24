...
class Garlic extends Balloon
{
    ...
    private function calculate_direction($start_x, $start_y, $target_x, $target_y)
    {
        if ($start_x == $target_x && $start_y == $target_y) {
            return -1; // No movement needed, already at the target
        }
        ...
        return -1; // Default case, should not happen if inputs are valid
    }

    public function findShortestPathDirection($board, $start_x, $start_y)
    {
        ...
        // If player is not found, return an invalid direction or handle accordingly
        if (!$foundPlayer) {
        ...
        // Ensure $parent[$currentY][$currentX] is not null before accessing its values
        while ($currentY !== null && $currentX !== null && $parent[$currentY][$currentX] !== null && ($parent[$currentY][$currentX][0] !== $start_x || $parent[$currentY][$currentX][1] !== $start_y)) {
        ...
        if ($currentY !== null && $currentX !== null && $parent[$currentY][$currentX] !== null) {
    ...
    {
        // Determine the shortest path direction to the player
        $direction = $this->findShortestPathDirection($board, $this->x, $this->y);

        // If a valid direction towards the player is found, choose it
        if ($direction !== -1 && $this->is_legal_move($direction, $board)) {
            $this->direction = $direction;
        } else {
            // If no valid direction found towards the player, choose a random legal direction
    ...
    public function move($board)
    {
        ...
        if ($this->move_percentage >= 100) {
            ...
            $this->x = $new_x;
            $this->move_percentage = 0;
            $this->choose_new_direction($board); // Adjust direction after move
        }
    }
}
